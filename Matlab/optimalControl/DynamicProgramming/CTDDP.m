...
%     [~,Vstate] = ode45(@valueFunction,linspace(t(end),0,nPoints),Vf,opt,OCP,@(T) interp1(t,x,T),@(T) interp1(t,u,T));
    ...
    [~,deltaX] = ode45(@linearDynamics,t,zeros(n,1),opt,...
        ...
        @(T) interp1(t,x,T,OCP.interp),...
        @(T) interp1(t,u,T,OCP.interp),...
        @(T) interp1(t,l,T,OCP.interp),...
        @(T) MatrixInterp(t,Kx,T,OCP.interp),...
        @(T) MatrixInterp(t,Kl,T,OCP.interp));
